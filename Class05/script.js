...
class CentralPoliciaNY extends EventEmitter {
    ...
    // --------------------------------------------------------
    reportarIncidente(lugar, villano, nivelPeligro) {
...
class SpiderMan {
    ...
    columpiarseAlLugar(alerta) {
        console.log(`🕸️ [Spider-Man]: ¡Mi sentido arácnido resuena! Me columpio hacia ${alerta.lugar} para detener a ${alerta.villano}.`);
    }
}
// ==========================================================
// DAILY BUGLE
...
class PeriodicoDailyBugle {
    publicarNoticia(alerta) {
        console.log(`📰 [Daily Bugle]: ¡ÚLTIMA HORA! J. Jonah Jameson exige fotos de ${alerta.villano} causando estragos en ${alerta.lugar}.`);
    }
}
// ==========================================================
// CREAR OBJETOS
...
//
centralPolicia.on("crimenEnProceso", (alerta) => {
...
//
centralPolicia.reportarIncidente("Times Square", "Duende Verde", "Alto");
// Otro crimen.
//
// El mismo evento vuelve a dispararse.
//
centralPolicia.reportarIncidente("Banco Central", "Rhino", "Medio");
