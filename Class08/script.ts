...
const DOM = {
    ...
    filtroUniverso: document.getElementById('filtroUniverso') as HTMLSelectElement,
...
const MAX_HEROES: number = 6;    // Límite máximo de héroes
let filtroActual: string = 'todos'; // Guarda el filtro seleccionado
let ordenAlfabetico: boolean = false; // Indica si está ordenado
...
function actualizarBotonReclutar(): void {
    if (heroes.length >= MAX_HEROES) {
...
function cargarHeroes(): Heroe[] {
    try {
        ...
        if (heroesJSON) {
            ...
            const heroesGuardados = JSON.parse(heroesJSON) as Heroe[];
...
function actualizarIdCounter(): void {
    if (heroes.length > 0) {
